...
export function DecisionScreen() {
  const { navigate, state, setState, setSpendPenalty } = useJourney();
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      const goalAmt = state.goal?.targetAmount ?? 5000;
      const pctOfGoal = goalAmt > 0 ? amt / goalAmt : 0;
      let verdict: Verdict = "SAFE";
      let tileDelta = 0;
      let daysDelta = 0;
      let reason = "";
      if (pctOfGoal > 0.4 || amt > safeToSpend * 0.15 || amt > 200) {
        verdict = "WRONG_TURN"; tileDelta = -3; daysDelta = Math.ceil(amt / 15);
        reason = `This $${amt} purchase is ${Math.round(pctOfGoal * 100)}% of your $${goalAmt} goal! A dangerous path — the demons of debt grow stronger!`;
      } else if (pctOfGoal > 0.15 || amt > safeToSpend * 0.05 || amt > 50) {
        verdict = "DETOUR"; tileDelta = -1; daysDelta = Math.ceil(amt / 30);
        reason = `This $${amt} spend is ${Math.round(pctOfGoal * 100)}% of your goal. A detour that delays you ~${daysDelta} days!`;
      } else {
        verdict = "SAFE"; tileDelta = 0; daysDelta = Math.ceil(amt / 50);
  ...
  const handleCommit = () => {
    ...
    if ((result.verdict === "DETOUR" || result.verdict === "WRONG_TURN") && td < 0) {
      setSpendPenalty({ verdict: result.verdict, tileDelta: td, reason: result.reason });
    }
    navigate("world");
  };
